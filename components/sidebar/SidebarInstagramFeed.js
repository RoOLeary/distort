    ...
    <div className="w-full p-5 bg-gray-50 sm:p-8 ">
      ...
      <div className="pt-6">
        <div className="grid grid-cols-3 gap-3 sm:gap-4 md:gap-6 lg:gap-4">
          ...
          {feed.map((item, index) => (
            <div key={index} className="relative z-0 w-full overflow-hidden transition duration-300 ease-in-out transform translate-y-0 shadow-sm cursor-pointer pt-full  group hover:-translate-y-1">
              <a href={item.href} className="absolute inset-0 z-10 w-full h-full shadow-md " />
